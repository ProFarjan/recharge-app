              ...
              <div class="row">
                <div class="col-md-12">
                  <form @submit.stop.prevent="rechargeRequest">
                    ...
                    <div class="form-group div_txt">
                      <label style="font-size: 18px;font-weight: 600;" for="amount">Payment Amount</label>
                      <b-form-input
                          id="amount"
                          v-model="fm.amount"
                          type="amount"
...
export default {
  ...
  created () {
    ...
    const operator = localStorage.getItem('BOP')
    if (operator) {
      this.operators = JSON.parse(operator)
    } else {
      this.$http.get('index.php?table=gsm&systype=banking', {
        headers: {
          'Accept': 'application/json',
          'Authorization': `Bearer ${this.user.accesToken}`
        }
        ...
        if (res.data.status === 'success') {
          ...
          localStorage.setItem('BOP', JSON.stringify(this.operators))
        }
      })
    }
    this.getLastItems()
