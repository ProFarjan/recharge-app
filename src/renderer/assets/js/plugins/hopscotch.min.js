 ...
 */
!(function (a, b) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = b() : typeof define === 'function' && define.amd ? define(b) : a.hopscotch = b()
}(this, function () {
  'use strict'
  var a, b, c, d, e, f, g, h, i, j, k, l, m, n = typeof Symbol === 'function' && typeof Symbol.iterator === 'symbol' ? function (a) {
      return typeof a
    } : function (a) {
    ...
    q = 'undefined',
    r = !1,
    s = (typeof jQuery === 'undefined' ? 'undefined' : n(jQuery)) !== q,
    t = !1,
    u = !1,
    v = /^[a-zA-Z]+[a-zA-Z0-9_-]*$/,
    w = {
  ...
  l = {
    ...
    hasClass: function (a, b) {
      var c
      return a.className ? (c = ' ' + a.className + ' ', c.indexOf(' ' + b + ' ') !== -1) : !1
    },
    getPixelValue: function (a) {
      var b = typeof a === 'undefined' ? 'undefined' : n(a)
      return b === 'number' ? a : b === 'string' ? parseInt(a, 10) : 0
    },
    valOrDefault: function (a, b) {
      return (typeof a === 'undefined' ? 'undefined' : n(a)) !== q ? a : b
    },
    invokeCallbackArrayHelper: function (a) {
      var b
      return Array.isArray(a) && (b = j[a[0]], typeof b === 'function') ? b.apply(this, a.slice(1)) : void 0
    },
    invokeCallbackArray: function (a) {
      var b, c
      if (Array.isArray(a)) {
        if (typeof a[0] === 'string') return h.invokeCallbackArrayHelper(a)
        for (b = 0, c = a.length; c > b; ++b) h.invokeCallback(a[b])
      }
    },
    invokeCallback: function (a) {
      return typeof a === 'function' ? a() : typeof a === 'string' && j[a] ? j[a]() : h.invokeCallbackArray(a)
    },
    ...
    getStepTarget: function (a) {
      ...
      if (typeof a.target === 'string') return h.getStepTargetHelper(a.target)
      if (Array.isArray(a.target)) {
        var c, d
        for (c = 0, d = a.target.length; d > c; c++) { if (typeof a.target[c] === 'string' && (b = h.getStepTargetHelper(a.target[c]))) return b }
        return null
      }
      return a.target
    },
    ...
    setPosition: function (a) {
      ...
      if (h.flipPlacement(a), h.normalizePlacement(a), c = k.offsetWidth, b = k.offsetHeight, h.removeClass(k, 'fade-in-down fade-in-up fade-in-left fade-in-right'), d = j.getBoundingClientRect(), i = a.isRtl ? d.right - c : d.left, a.placement === 'top') e = d.top - b - this.opt.arrowWidth, f = i
      else if (a.placement === 'bottom') e = d.bottom + this.opt.arrowWidth, f = i
      else if (a.placement === 'left') e = d.top, f = d.left - c - this.opt.arrowWidth
      else {
        if (a.placement !== 'right') throw new Error('Bubble placement failed because step.placement is invalid or undefined!')
        e = d.top, f = d.right + this.opt.arrowWidth
      }
      g = a.arrowOffset !== 'center' ? h.getPixelValue(a.arrowOffset) : a.arrowOffset, g ? a.placement === 'top' || a.placement === 'bottom' ? (l.style.top = '', g === 'center' ? l.style[m] = Math.floor(c / 2 - l.offsetWidth / 2) + 'px' : l.style[m] = g + 'px') : (a.placement === 'left' || a.placement === 'right') && (l.style[m] = '', g === 'center' ? l.style.top = Math.floor(b / 2 - l.offsetHeight / 2) + 'px' : l.style.top = g + 'px') : (l.style.top = '', l.style[m] = ''), a.xOffset === 'center' ? f = d.left + j.offsetWidth / 2 - c / 2 : f += h.getPixelValue(a.xOffset), a.yOffset === 'center' ? e = d.top + j.offsetHeight / 2 - b / 2 : e += h.getPixelValue(a.yOffset), a.fixedElement || (e += h.getScrollTop(), f += h.getScrollLeft()), k.style.position = a.fixedElement ? 'fixed' : 'absolute', k.style.top = e + 'px', k.style.left = f + 'px'
    },
    render: function (a, b, c) {
      ...
      }, typeof d === 'function') r.innerHTML = d(q)
      else if (typeof d === 'string') {
        if (!m.templates || typeof m.templates[d] !== 'function') throw new Error('Bubble rendering failed - template "' + d + '" is not a function.')
        r.innerHTML = m.templates[d](q)
      } else if (f) r.innerHTML = f(q)
      else {
        if (!m.templates || typeof m.templates[o] !== 'function') throw new Error('Bubble rendering failed - template "' + o + '" is not a function.')
        r.innerHTML = m.templates[o](q)
      }
      var s, t = r.children,
        u = t.length
      for (p = 0; u > p; p++) s = t[p], h.hasClass(s, 'hopscotch-arrow') && (this.arrowEl = s)
      return r.style.zIndex = typeof a.zindex === 'number' ? a.zindex : '', this._setArrow(a.placement), this.hide(!1), this.setPosition(a), c && c(!a.fixedElement), this
    },
    ...
    _setArrow: function (a) {
      h.removeClass(this.arrowEl, 'down up right left'), a === 'top' ? h.addClass(this.arrowEl, 'down') : a === 'bottom' ? h.addClass(this.arrowEl, 'up') : a === 'left' ? h.addClass(this.arrowEl, 'right') : a === 'right' && h.addClass(this.arrowEl, 'left')
    },
    _getArrowDirection: function () {
      return this.placement === 'top' ? 'down' : this.placement === 'bottom' ? 'up' : this.placement === 'left' ? 'right' : this.placement === 'right' ? 'left' : void 0
    },
    show: function () {
      var a = this,
    ...
    _handleBubbleClick: function (a) {
      ...
      if (c = b(d), c === 'cta') this.opt.isTourBubble || m.getCalloutManager().removeCallout(this.currStep.id), this.currStep.onCTA && h.invokeCallback(this.currStep.onCTA)
      else if (c === 'next') m.nextStep(!0)
      else if (c === 'prev') m.prevStep(!0)
      else if (c === 'close') {
    ...
    init: function (a) {
      ...
      this.element = f, e = {
        ...
        showNumber: !0,
        isTourBubble: !0
      }, a = (typeof a === 'undefined' ? 'undefined' : n(a)) === q ? {} : a, h.extend(e, a), this.opt = e, f.className = 'hopscotch-bubble animated', e.isTourBubble ? (d = m.getCurrTour(), d && h.addClass(f, 'tour-' + d.id)) : h.addClass(f, 'hopscotch-callout no-number'), b = function () {
        ...
        document.readyState === 'complete' && (document.detachEvent('onreadystatechange', c), window.detachEvent('onload', c), document.body.appendChild(f))
    ...
    var d, k, p, t, u, w, x, y, z = this,
      ...
      E = function (a) {
        return typeof p === 'undefined' ? l[a] : h.valOrDefault(p[a], l[a])
      },
      F = function () {
        var a
        return a = !t || u < 0 || u >= t.steps.length ? null : t.steps[u]
      },
      G = function () {
        z.nextStep()
      },
      H = function (a) {
        ...
        t >= v && (t <= v + E('scrollTopMargin') || w >= u) ? a && a() : E('smoothScroll') ? (typeof YAHOO === 'undefined' ? 'undefined' : n(YAHOO)) !== q && n(YAHOO.env) !== q && n(YAHOO.env.ua) !== q && n(YAHOO.util) !== q && n(YAHOO.util.Scroll) !== q ? (b = YAHOO.env.ua.webkit ? document.body : document.documentElement, d = YAHOO.util.Easing ? YAHOO.util.Easing.easeOut : void 0, c = new YAHOO.util.Scroll(b, {
          scroll: {
            to: [0, x]
          }
        }, E('scrollDuration') / 1e3, d), c.onComplete.subscribe(a), c.animate()) : s ? jQuery('body, html').animate({
          scrollTop: x
        }, E('scrollDuration'), a) : (x < 0 && (x = 0), e = v > t ? -1 : 1, f = Math.abs(v - x) / (E('scrollDuration') / 10), (g = function () {
          var b = h.getScrollTop(),
            c = b + e * f
          return e > 0 && c >= x || e < 0 && x >= c ? (c = x, a && a(), void window.scrollTo(0, c)) : (window.scrollTo(0, c), h.getScrollTop() === b ? void (a && a()) : void setTimeout(g, 10))
      ...
      K = function (a) {
        var b, c, d, e = {}
        for (b in a) a.hasOwnProperty(b) && b !== 'id' && b !== 'steps' && (e[b] = a[b])
    ...
    this.getCalloutManager = function () {
      return (typeof k === 'undefined' ? 'undefined' : n(k)) === q && (k = new c()), k
      ...
      for (f in i) {
        if (!a || a === f) {
          if (b) { for (c = i[f], d = 0, e = c.length; e > d; ++d) c[d].fromTour && (c.splice(d--, 1), --e) } else i[f] = []
        }
      }
      return this
    }, this.registerHelper = function (a, b) {
      typeof a === 'string' && typeof b === 'function' && (j[a] = b)
      ...
      var b = typeof a === 'undefined' ? 'undefined' : n(a)
      return b === 'string' ? (o = a, f = void 0) : b === 'function' && (f = a), this
    }, this.setEscaper = function (a) {
      return typeof a === 'function' && (g = a), this
    }, O.call(this, a)
  }, m = new a(),
  (function () {
    var a = {}
    a.escape = function (a) {
      return g ? g(a) : a == null ? '' : ('' + a).replace(new RegExp("[&<>\"']", 'g'), function (a) {
      ...
      var f = b.i18n,
        ...
        h = b.step,
        i = b.tour
      return e += '\n<div class="hopscotch-bubble-container" style="width: ' + ((d = h.width) == null ? '' : d) + 'px; padding: ' + ((d = h.padding) == null ? '' : d) + 'px;">\n  ', i.isTour && (e += '<span class="hopscotch-bubble-number">' + ((d = f.stepNum) == null ? '' : d) + '</span>'), e += '\n  <div class="hopscotch-bubble-content">\n    ', h.title !== '' && (e += '<h3 class="hopscotch-title">' + ((d = c(h.title, i.unsafe)) == null ? '' : d) + '</h3>'), e += '\n    ', h.content !== '' && (e += '<div class="hopscotch-content">' + ((d = c(h.content, i.unsafe)) == null ? '' : d) + '</div>'), e += '\n  </div>\n  <div class="hopscotch-actions">\n    ', g.showPrev && (e += '<button class="hopscotch-nav-button prev hopscotch-prev">' + ((d = f.prevBtn) == null ? '' : d) + '</button>'), e += '\n    ', g.showCTA && (e += '<button class="hopscotch-nav-button next hopscotch-cta">' + ((d = g.ctaLabel) == null ? '' : d) + '</button>'), e += '\n    ', g.showNext && (e += '<button class="hopscotch-nav-button next hopscotch-next">' + ((d = f.nextBtn) == null ? '' : d) + '</button>'), e += '\n  </div>\n  ', g.showClose && (e += '<button class="hopscotch-bubble-close hopscotch-close">' + ((d = f.closeTooltip) == null ? '' : d) + '</button>'), e += '\n</div>\n<div class="hopscotch-bubble-arrow-container hopscotch-arrow">\n  <div class="hopscotch-bubble-arrow-border"></div>\n  <div class="hopscotch-bubble-arrow"></div>\n</div>\n'
    }
  }.call(m))
  var y = m
  return y
